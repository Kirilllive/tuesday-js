...
tuesday.addEventListener('mouseup',function(e){
    if(tue_screen_control && e.pageX > tuesday.offsetWidth/3 ){if(check_choice(story_json[tue_story][scene].dialogs)){go_story()}}
    else if( check_choice(story_json[tue_story][scene].dialogs) && tue_screen_control){back_story()};
