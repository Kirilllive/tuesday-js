...
tuesday.addEventListener('creation_dialog',()=>{
    if(story_json.parameters.text_panel.speech||arr_dialog.speech){
        const speech=new SpeechSynthesisUtterance(dialog_text);
        if (synth.speaking){synth.cancel()}
        let voice=(arr_dialog.speech)?arr_dialog.speech[languare]:story_json.parameters.text_panel.speech[languare];
