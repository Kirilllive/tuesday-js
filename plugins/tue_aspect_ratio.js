function TueAspectRatio(){
    ...
    if( rect.height==0 || rect.width==0 ){
        var s=(story_json.parameters.resolutions[4])?((tue_size[0]/window.innerWidth)>(tue_size[1]/window.innerHeight))?window.innerWidth/tue_size[0]:window.innerHeight/tue_size[1]:((tue_size[0]/window.innerWidth)<(tue_size[1]/window.innerHeight))?window.innerWidth/tue_size[0]:window.innerHeight/tue_size[1];
        tuesday.style.left=(window.innerWidth-(tue_size[0]*s))/2+'px';
        tuesday.style.top=(window.innerHeight-(tue_size[1]*s))/2+'px';
    }else{
        var s=(story_json.parameters.resolutions[4])?((tue_size[0]/rect.width)<(tue_size[1]/rect.height))?rect.width/tue_size[0]:rect.height/tue_size[1]:((tue_size[0]/rect.width)>(tue_size[1]/rect.height))?rect.width/tue_size[0]:rect.height/tue_size[1];
